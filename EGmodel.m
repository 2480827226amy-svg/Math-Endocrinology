%% Dr. Erica Graham's original 2017 model
% Reduced to three subsystems; contains no other alterations
...
% Solve the ODE
[t, y] = ode15s(@(t,y) f(t,y), tspan, y0, opts);

yyaxis left
plot(t,y(:,4),"LineWidth",2,"Color","blue","LineStyle","-")
ylabel ('LH (\mu g/L)')

yyaxis right
plot(t,y(:,12),"LineWidth",2,"Color","red","LineStyle","-")
ylabel ('P4 (ng/mL)')

xlabel('Time (days)')
title('Pre-T Menstrual Cycle')
legend('LH','P4')
fontsize(.75, "centimeters")
...
F2 = SLH / D;
% Tdose= 7142858; 
%dosage of testosterone converted to nanograms/L per day;
% SerumT0 = 273.67 + Tdose;
SerumT0= 273.67;
