%% Dosing function iteration 2
% Triangular pulse function
% Unfinished (unincorporated into model) as of 5/20/25
...
[t, y] = ode45(@(t, y) f(t, y, tdose(t)), tspan, y0);
%Triangle pulse set up
function t = triPulse(A,B,x)
C = (A+B)/2;
if x <= A
  t = 0;
elseif x >=B
...
function level = tdose(t)
level = zeros(size(t));
for i = 1:1:length(t)
c = floor((t(i))./7).*7;
d = c+(7);
height = (2*10^6)*7;
level(i) = height.*triPulse(c,d,t(i));
end
end
...
% Serum T, T
dydx(9) = t0 - deltaT * SerumT + (((t1 * G1) * (F1 + cTF2 * F2)) + t2 * G1 * G2 * F1) * (dphi + tau1 * domega + tau2 * S * dlambda + tau3 * (1 - ((dphi + domega + dlambda) / psi))) + level;
